...
class HistoryMenu extends AbstractController
{
    ...
    public function load(Request $request): Response
    {
        ...
        $history = array_map(function ($item) use ($user) {
            $item['url'] = $this->generateUrl('app_show_episode', ['id' => $item['id'], 'slug' => 'slug-'.$item['slug'], 'seasonNumber' => $item['seasonNumber'], 'episodeNumber' => $item['episodeNumber']]);
